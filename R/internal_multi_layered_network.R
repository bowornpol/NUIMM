...
# --- Taxonomy Cleaner & Family Extractor ---
clean_and_get_family <- function(tax_string) {
  ...
  if (grepl("g__", tax_string)) {
    ...
    if (!is.na(s_part) && nchar(s_part) > 3) result$clean <- paste(g_part, s_part, sep = " ") else if (!is.na(g_part)) result$clean <- g_part
      ...
      tryCatch(
        {
          ...
          vis_plot <- visNetwork(nodes_df, edges_df, width = "100%", height = "100vh", main = list(text = "💡 Tip: Scroll to zoom. Drag nodes to perfect layout.", style = "color:#475569; font-size:14px;")) |>
            ...
            visPhysics(enabled = TRUE, solver = "forceAtlas2Based", forceAtlas2Based = list(avoidOverlap = 1, springConstant = 0.08, centralGravity = 0.005)) |>
            visExport(type = "png", label = "💾 SAVE NETWORK", style = "position:absolute; right:30px; top:30px; background:#f1f5f9; color:#475569; padding:12px 24px; border-radius:10px; border:1px solid #cbd5e1; cursor:pointer; font-weight:bold; z-index:1000;")
