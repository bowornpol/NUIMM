#' Pathfinding using Dijkstra's algorithm
#'
#' @details
#' This function calculates the shortest path between two nodes using Dijkstra's
#' algorithm, contextualized within a comprehensive overlay visualization.
#'
#' @param multi_layered_network_file Path to input CSV/TSV.
#' @param source_node Name of the starting node.
#' @param target_node Name of the ending node.
#' @param output_directory Path to save results.
#' @param file_type "csv" or "tsv".
#' @param visualize Logical. If TRUE, saves PDF and PNG overlay network plots. Defaults to TRUE.
#' @param layout_method Character. Layout algorithm ("kk" for Kamada-Kawai suggested).
#' @param node_colors Named character vector. Visual coloring scheme by node type.
#' @param node_shapes Named numeric vector. Base shapes for rendering biological layers.
#' @param plot_width Numeric. Output width size in inches.
#' @param plot_height Numeric. Output height size in inches.
#' @param plot_dpi Numeric. Output PNG resolution.
#' @export
utils::globalVariables(c("from", "to", "edge_score", "x", "y", "xend", "yend", "type", "name", "is_path"))

find_path <- function(
  multi_layered_network_file,
  source_node,
  target_node,
  output_directory,
  file_type = c("csv", "tsv"),
  visualize = TRUE,
  layout_method = "kk", 
  node_colors = c("Microbe" = "#D55E00", "Pathway" = "#0072B2", "Metabolite" = "#009E73"),
  node_shapes = c("Microbe" = 24, "Pathway" = 21, "Metabolite" = 22),
  plot_width = 12, 
  plot_height = 10, 
  plot_dpi = 600
) {
  file_type <- match.arg(file_type)
  message("Starting shortest pathfinding using Dijkstra's algorithm.")

  if (!dir.exists(output_directory)) dir.create(output_directory, recursive = TRUE)

  message("\n1. Loading network...")
  network_data <- read_input_file(multi_layered_network_file, file_type = file_type, stringsAsFactors = FALSE)

  req <- c("Feature1", "Feature2", "edge_score")
  if (!all(req %in% colnames(network_data))) stop("Missing columns: ", paste(req, collapse = ", "))

  network_data$edge_score <- as.numeric(network_data$edge_score)
  network_data$edge_score_abs <- abs(network_data$edge_score)

  g <- igraph::graph_from_data_frame(d = network_data, directed = FALSE)

  igraph::E(g)$weight <- sapply(igraph::E(g)$edge_score_abs, function(w) {
    if (is.na(w)) Inf else if (w < 1) 1 / w else 1 / (w + 0.1)
  })

  if (!source_node %in% igraph::V(g)$name) stop("Source node not found.")
  if (!target_node %in% igraph::V(g)$name) stop("Target node not found.")

  message("\n2. Finding shortest path...")
  result <- igraph::shortest_paths(g, from = source_node, to = target_node, weights = igraph::E(g)$weight, output = "both")
  path_vertices <- result$vpath[[1]]
  path_edges <- result$epath[[1]]

  if (length(path_vertices) > 1) {
    message("  Path found with ", length(path_edges), " steps.")

    safe_from <- gsub("[^A-Za-z0-9]", "_", source_node)
    safe_to <- gsub("[^A-Za-z0-9]", "_", target_node)
    base_name <- paste0("path_", safe_from, "_to_", safe_to)

    edge_df <- igraph::as_data_frame(g, what = "edges")[path_edges, ]
    path_df <- dplyr::select(edge_df, Source = from, Target = to, edge_score = edge_score)
    write.csv(path_df, file.path(output_directory, paste0(base_name, ".csv")), row.names = FALSE)

    if (visualize) {
      message("\n3. Generating network visualization...")
      if (!requireNamespace("ggplot2", quietly = TRUE)) stop("Install 'ggplot2'.")

      set.seed(42)
      
      igraph::V(g)$type <- sapply(igraph::V(g)$name, function(x) {
        if (grepl("d__|p__|c__|o__|f__|g__|s__|Bacteria", x)) "Microbe"
        else if (grepl("^ko[0-9]+", x)) "Pathway"
        else "Metabolite"
      })
      igraph::V(g)$type <- factor(igraph::V(g)$type, levels = c("Microbe", "Pathway", "Metabolite"))

      igraph::V(g)$is_path <- igraph::V(g)$name %in% names(path_vertices)
      igraph::E(g)$is_path <- FALSE
      igraph::E(g)[path_edges]$is_path <- TRUE

      p <- ggraph::ggraph(g, layout = layout_method) +
        ggraph::geom_edge_link(ggplot2::aes(color = is_path, width = is_path, alpha = is_path), show.legend = FALSE) +
        ggraph::scale_edge_color_manual(values = c("TRUE" = "black", "FALSE" = "gray85")) +
        ggraph::scale_edge_width_manual(values = c("TRUE" = 1.5, "FALSE" = 0.3)) +
        ggraph::scale_edge_alpha_manual(values = c("TRUE" = 1.0, "FALSE" = 0.2)) +
        ggraph::geom_node_point(ggplot2::aes(fill = type, shape = type, size = is_path, alpha = is_path), color = "black", stroke = 0.5) +
        ggplot2::scale_fill_manual(name = "Node Type", values = node_colors) +
        ggplot2::scale_shape_manual(name = "Node Type", values = node_shapes) +
        ggplot2::scale_size_manual(values = c("TRUE" = 8, "FALSE" = 3), guide = "none") +
        ggplot2::scale_alpha_manual(values = c("TRUE" = 1.0, "FALSE" = 0.3), guide = "none") +
        ggraph::geom_node_text(ggplot2::aes(label = ifelse(is_path, name, "")), repel = TRUE, size = 4, fontface = "bold", family = "sans") +
        ggplot2::theme_void(base_family = "sans") +
        ggplot2::theme(
          legend.position = "right",
          legend.title = ggplot2::element_text(face = "bold", size = 12),
          legend.text = ggplot2::element_text(size = 10),
          plot.margin = ggplot2::margin(10, 10, 10, 10)
        )

      pdf_path <- file.path(output_directory, paste0(base_name, ".pdf"))
      png_path <- file.path(output_directory, paste0(base_name, ".png"))
      ggplot2::ggsave(pdf_path, plot = p, width = plot_width, height = plot_height, device = cairo_pdf)
      ggplot2::ggsave(png_path, plot = p, width = plot_width, height = plot_height, dpi = plot_dpi, bg = "white")
      message("  Saved visualization to: ", png_path)
    }
    invisible(NULL)
  } else {
    message("  No path found.")
    invisible(NULL)
  }
}